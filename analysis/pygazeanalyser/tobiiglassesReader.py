...
from detectors4c import blink_detection, fixation_detection, saccade_detection
def replace_missing(value, missing=0.0):
	...
	if value.replace(' ','') == '-1.000':
		return missing
	else:
		return float(value)

def read_tobii4c(filename, start, stop=None, missing=0.0, debug=False):
    ...
    for i in range(len(raw)):
            ...
            # string to list
            #line = raw[i].replace('\n','').replace('\r','').replace('-1.000','0.0').split('\t')
            line = raw[i].decode("utf-8")
            line = raw[i].replace('\n','').split('\t')
            print line[1].decode('utf-16').decode('unicode_escape')
            # check if trial has already started
            if started:
                # only check for stop if there is one
                if stop != None:
                    if (line[1] == 'MSG' and stop in line[2]) or i == len(raw)-1:
                        #print line[2]
                        started = False
                        trialend = True
            ...
            if started:
                ...
                # 501.8202, 546.3453, 609.3405, 623.2287, 16.7916, 0.5539, 0.5209'
                    ...
                    try:
                        ...
                        time.append(float(line[0])-starttime)
                        trackertime.append(float(line[0]))
...
#read_tobii4c("AlexTobiiGlasses.tsv","start_recording","stop_recording",debug=False)
print read_tobii4c("AlexTobiiGlasses.tsv", "start_recording", stop="stop_recording", missing=0.0, debug=True)