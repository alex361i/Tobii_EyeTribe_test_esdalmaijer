...
def read_tobiitx(filename, start, stop=None, missing=0.0, debug=False):
    ...
    for i in range(len(raw)):
            ...
            if started:
                ...
                if stop != None:
                    if (line[0] == 'MSG' and stop in line[2]) or i == len(raw)-1:
                        started = False
                        trialend = True
...
#read_eyetribe("tobiiCC.tsv","start_recording","stop_recording",debug=False)
#read_tobiitx("Alin.tsv", "PUPIL_TRIALSTART", stop="pupdata_stop", missing=0.0, debug=True)
#read_tobiitx("Alin.tsv", "target_on", stop="target_fixated", missing=0.0, debug=True)