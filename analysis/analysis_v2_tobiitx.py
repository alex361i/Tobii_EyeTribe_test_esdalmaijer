...
read_data = {'tobiitx':read_tobiitx}
...
for trackertype in ['tobiitx']:    
    ...
    if trackertype == 'tobiitx':
        ...
        pupilsamples = 180
        baselinpupilsamples = 18
        samplingrate = 90
        '''
        #2 sec for the tx300
        pupilsamples = 600
        baselinpupilsamples = 55
        samplingrate = 300
...
#                # # # # #
    ...
    if DOINTSAMPLETIME:
        ...
        if trackertype == 'tobiitx':
            #under 3 ms
            N0 = numpy.sum((1.00 < ait).astype(int) + (ait <= 2.00).astype(int) == 2)
            # 300 hz
            ...
            txtfile = open(os.path.join(outputdir, "intersample_times.txt"), 'w')
            # calculate data
            header = ['intersample time', '0-2', '3', '4', '5','35-50', 'higher', 'inttimes_total', 'samples_missing', 'samples_total']
            content = ['',N0, N3, N4, N5, Nh, itotal, Nmissing, total]
            # write data
            txtfile.write(OUTSEP.join(header) + '\n')
            txtfile.write(OUTSEP.join(map(str, content)) + '\n')
            # close textfile
