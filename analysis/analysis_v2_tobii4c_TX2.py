...
from pygazeanalyser.tobii4creader import read_tobii4c
from pygazeanalyser.tobiitxreader import read_tobiitx
...
ACCTHRESH = 9500 # degrees per second**2
SCREENDIST = 65.0 # cm
SCREENSIZE = (50.9,28.6) # cm
PIXPERCM = numpy.mean([DISPSIZE[0]/SCREENSIZE[0], DISPSIZE[1]/SCREENSIZE[1]])
'''
# saccade detection tobii4c
MAXDISP = 0.25 # degrees of visual angle; maximal horizontal dispersion from fixation
MAXENDERR = 0.75 # degrees of visual angle; maximal distance from target
MINSACDUR = 20 # milliseconds
FIXPOS = (DISPSIZE[0] * 0.25, DISPSIZE[1] * 0.5)
...
PLOTCOLS = {    'raw':        '#2e3436',
            ...
            'black':        '#204a87',
            'tobii4c':     '#204a87',#blue
            'tobiitx300':        '#a40000',#red
            'significant':    '#babdb6',
            'pupscreenonset':'#4e9a06'
            }

# checks
if not os.path.isdir(DATADIRTX):
    raise Exception("ERROR: no data directory found! (expecting '%s')" % DATADIRTX)
...
read_data = {'tobii4c':read_tobii4c, 'tobiitx300':read_tobiitx}
...
for trackertype in ['tobii4c','tobiitx300']:    
    ...
    if trackertype == 'tobii4c':
        DATAFILE = os.path.join(DATADIRTX, '%s_TOBII_4c_output.tsv' % PPNAME)
    elif trackertype == 'tobiitx300':
        DATAFILE = os.path.join(DATADIRTX, '%s_TOBII_tx_output.tsv' % PPNAME)
    ...
    if trackertype == 'tobii4c':
        ...
        pupilsamples = 180
        baselinpupilsamples = 13
        samplingrate = 90
    elif trackertype == 'tobiitx300':
        #2 sec for the tx300
        pupilsamples = 600
        baselinpupilsamples = 60
        samplingrate = 300
    ...
    if DOVALIDATION:
        ...
        # ACCURACY AND PRECISION #accuracy is the average difference between what was recorded gaze position and the actual gaze position
        # calculate values #precision is the root means square of the distance (in degrees of visual angle) between successive samples.(range from 0.1 - 1 degree)
        RMS = {}
        acc = {}
        for d in ['x', 'y']:
    ...
    if DOPUPILOMETRY:
        ...
        # correct STUPID mistake (forgot to set pupil measure to DIAMETER)
            ...
            for i in range(len(pupildata)):
                ...
                #print pupildata[i]['events']['msg'][0]
                if 'white' in msg:
                    condition = 'white'
                else:
                    condition = 'black'
            ...
            for condition in baseline.keys():
                for i in range(len(baseline[condition])):
                    ...
                    ######testing
                    # interpolate missing
                    baseline[condition][i] = pyenalysis.interpolate_missing(baseline[condition][i], mindur=1, invalid=-1.000)
                    pupresponse[condition][i] = pyenalysis.interpolate_missing(pupresponse[condition][i], mindur=1, invalid=-1.000)
                    # interpolate blinks
                    baseline[condition][i] = pyenalysis.interpolate_blink(baseline[condition][i], mode='auto', invalid=-1.000, edfonly=False)
                    pupresponse[condition][i] = pyenalysis.interpolate_blink(pupresponse[condition][i], mode='auto', invalid=-1.000, edfonly=False)
                    # hampel
                    baseline[condition][i] = pyenalysis.hampel(baseline[condition][i])
                    pupresponse[condition][i] = pyenalysis.hampel(pupresponse[condition][i])
                    # proportionalize
...
#                # # # # #
    ...
    if DOINTSAMPLETIME:
        ...
        if trackertype == 'tobii4c':
            ...
            # 11.10 ms inter-sample time (90 Hz)
            N11 = numpy.sum((11.10 < ait).astype(int) + (ait <= 14.67).astype(int) == 2)
            ...
            content = ['', Nlower,N11, N16, N33, N50, Nhigher, itotal, Nmissing, total]
            # write data
            txtfile.write(OUTSEP.join(header) + '\n')
