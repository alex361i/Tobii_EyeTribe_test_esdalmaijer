...
from pygazeanalyser.tobii4txreader import read_tobiitx
...
read_data = {'tobiitx300':read_tobiitx}
...
for trackertype in ['tobiitx300']:    
    ...
    if DOIMAGES:
        ...
        for i in range(len(imagedata)):
            ...
            fixfile.close()
            # plot raw data
            fig = gazeplotter.fraw(imagedata[i]['x'], imagedata[i]['y'], DISPSIZE, imagefile=IMAGES[i], savefilename=os.path.join(outputdir, "image_%s_raw.png" % imgname))
            pyplot.close(fig)
            # plot fixations
...
#                # # # # #
    ...
    if DOINTSAMPLETIME:
        ...
        if trackertype == 'tobiitx300':
            ...
            # 300 hz
            N3 = numpy.sum((3.33 < ait).astype(int) + (ait <= 6.66).astype(int) == 2)
            # 150 hz
            N4 = numpy.sum((6.00 < ait).astype(int) + (ait <= 9.33).astype(int) == 2)
            # 3 ms
            N5 = numpy.sum((10.10 < ait).astype(int) + (ait <= 13.67).astype(int) == 2)
            # higher
            Nh = numpy.sum((15 < ait).astype(int))
            # all missing data
            Nmissing = numpy.sum((combined == 0.0).astype(int))
            ...
            # calculate data
            header = ['intersample time', '0-15', '16', '19-35', '35-50', 'higher', 'inttimes_total', 'samples_missing', 'samples_total']
            content = ['', N3, N4, N5, Nh, itotal, Nmissing, total]
            # write data
            txtfile.write(OUTSEP.join(header) + '\n')
            txtfile.write(OUTSEP.join(map(str, content)) + '\n')
            # close textfile
