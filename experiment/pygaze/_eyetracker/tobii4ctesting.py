...
class TobiiTracker(BaseEyeTracker):
    ...
    def calibrate(self, calibrate=True, validate=True):
        ...
        for i in range(0,len(self.controller.gazeData)-1):
            ist[i] = (self.controller.gazeData[i+1]['system_time_stamp'] - self.controller.gazeData[i]['device_time_stamp']) / 1000.0
        ...
        self.controller.eyetracker.unsubscribe_from(tr.EYETRACKER_GAZE_DATA,self.controller.on_gazedataAll)
...
class TobiiController:
    ...
    def __init__(self, disp):
        ...
        #time sychronized between eyetracker and system
        
        
        with open("Here we should have the license file from tobi", "rb") as f:
            license = f.read()
...
#                win.flip()
    ...
    def startTracking(self):
        ...
        self.eventData = []
        self.eyetracker.subscribe_to(tr.EYETRACKER_GAZE_DATA,self.on_gazedata,as_dictionary=True)
        self.eyetracker.subscribe_to(tr.EYETRACKER_GAZE_DATA,self.on_gazedataAll,as_dictionary=True)
    ...
    def stopTracking(self):
        ...
        self.eyetracker.unsubscribe_from(tr.EYETRACKER_GAZE_DATA, self.on_gazedataAll)
        self.eyetracker.unsubscribe_from(tr.EYETRACKER_GAZE_DATA, self.on_gazedata)
    ...
    def flushData(self):
        ...
        for e in self.eventData:
            self.datafile.write(formatstr % ((e[0]/1000.0),e[1]))
            print ("event writing error")
...
def q_reader(queue):
    ...
    while True:
        message = queue.get()
        if (message == "DONE"):
            print ("Everything is out of the queue")
            break
            
def q_writer(times,queue):
        #Writing to queue
    for element in xrange(0,times):
        queue.put(element)
    queue.put("DONE")
    

'''
{'right_pupil_validity': 1, 'right_gaze_point_on_display_area': (0.4584959149360657, 0.5215896964073181), 'left_gaze_origin_validity': 1, 'system_time_stamp': 445924277767L, 'right_gaze_origin_in_user_coordinate_system': (-16.11235237121582, -16.06415557861328, 468.7225036621094), 'left_gaze_point_in_user_coordinate_system': (-16.363758087158203, 104.4046859741211, -3.9999918937683105), 'left_gaze_origin_in_user_coordinate_system': (-78.24421691894531, -20.32554817199707, 462.42108154296875), 'left_pupil_validity': 1, 'right_pupil_diameter': 2.21417236328125, 'left_gaze_origin_in_trackbox_coordinate_system': (0.7406595349311829, 0.5951764583587646, 0.1388685256242752), 'right_gaze_point_in_user_coordinate_system': (-14.318913459777832, 105.7972412109375, -4.0), 'left_pupil_diameter': 2.3138580322265625, 'right_gaze_origin_validity': 1, 'left_gaze_point_validity': 1, 'right_gaze_point_validity': 1, 'left_gaze_point_on_display_area': (0.45256882905960083, 0.5287689566612244), 'right_gaze_origin_in_trackbox_coordinate_system': (0.547992467880249, 0.5791040658950806, 0.13854876160621643), 'device_time_stamp': 1539802269L}
'''
